...
namespace AC.Attribute
{
    ...
    public class ConditionFieldPropertyDrawer : PropertyDrawer
    {
        private ConditionFieldAttribute Conditional => _conditional ?? (_conditional = attribute as ConditionFieldAttribute);
        private ConditionFieldAttribute _conditional;
        ...
        private void Initialize(SerializedProperty property)
        {
            ...
            {
                SerializedProperty[] serializedPropertiesFieldToCheck = new SerializedProperty[Conditional.FieldToCheck.Length];
                for (int i=0; i< Conditional.FieldToCheck.Length; i++)
                {
                    serializedPropertiesFieldToCheck[i] = ConditionalFieldUtility.FindRelativeProperty(property, Conditional.FieldToCheck[i]);
                }
                //_conditionalToTarget.Add(property, ConditionalFieldUtility.FindRelativeProperty(property, Conditional.FieldToCheck));
                _conditionalToTarget.Add(property, serializedPropertiesFieldToCheck);
            }
            else
            {
                SerializedProperty[] serializedPropertiesFieldToCheck = new SerializedProperty[Conditional.FieldToCheck.Length + _conditionalToTarget[property].Length];
                int count = 0;
                while(count< _conditionalToTarget[property].Length)
                {
                    serializedPropertiesFieldToCheck[count] = _conditionalToTarget[property][count];
                }
                while (count < _conditionalToTarget[property].Length + Conditional.FieldToCheck.Length)
                {
                    serializedPropertiesFieldToCheck[count] = ConditionalFieldUtility.FindRelativeProperty(property, Conditional.FieldToCheck[count - _conditionalToTarget[property].Length]);
                }
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            Initialize(property);
            _toShow = true;
            for(int i =0; i< _conditionalToTarget[property].Length; i++)
            {
                if(!ConditionalFieldUtility.PropertyIsVisible(_conditionalToTarget[property][i], Conditional.Inverse, Conditional.CompareValues))
                {
                    _toShow = false;
                    break;
                }
        ...
        private void GetPropertyDrawerType(SerializedProperty property)
        {
            ...
            try
            {
                _genericAttribute = (PropertyAttribute)fieldInfo.GetCustomAttributes(typeof(PropertyAttribute), false)
                    .FirstOrDefault(a => !(a is ConditionFieldAttribute));
    ...
    public static class ConditionalFieldUtility
    {
        ...
        public static bool BehaviourPropertyIsVisible(MonoBehaviour behaviour, string propertyName, ConditionFieldAttribute appliedAttribute)
        {
            for (int i = 0; i < appliedAttribute.FieldToCheck.Length; i++)
            {
                if (string.IsNullOrEmpty(appliedAttribute.FieldToCheck[i])) return true;
            }
            

            var so = new SerializedObject(behaviour);
            var property = so.FindProperty(propertyName);
            var targetProperty = new SerializedProperty[appliedAttribute.FieldToCheck.Length];
            for (int i = 0; i < appliedAttribute.FieldToCheck.Length; i++)
            {
                targetProperty[i] = FindRelativeProperty(property, appliedAttribute.FieldToCheck[i]);
            }
            // var targetProperty = FindRelativeProperty(property, appliedAttribute.FieldToCheck);
            bool check = true;
            for (int i = 0; i < appliedAttribute.FieldToCheck.Length; i++)
            {
                check = check && PropertyIsVisible(targetProperty[i], appliedAttribute.Inverse, appliedAttribute.CompareValues);
            }
